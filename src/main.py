...
class DifferenceGame:
    ...
    def create_ui(self):
        ...
        self.reveal_btn.grid(row=0, column=0, padx=10)
        ...
        image_frame = tk.Frame(self.root, bg="#101010")
        image_frame.pack(pady=15)

        left_frame = tk.Frame(image_frame, bg="#202020", padx=10, pady=10)
        left_frame.grid(row=0, column=0, padx=18)

        right_frame = tk.Frame(image_frame, bg="#202020", padx=10, pady=10)
        ...
        self.left_label = tk.Label(left_frame, bg="#111111")
        self.left_label.pack()

        self.right_label = tk.Label(right_frame, bg="#111111")
        self.right_label.pack()
        self.right_label.bind("<Button-1>", self.check_click)
    ...
    def start_game(self):
        try:
            ...
            image = cv2.resize(image, (650, 420))
            ...
            self.update_images()
            self.update_status("Find all 5 hidden differences.")

        except Exception:
            messagebox.showerror(
                "Image Error",
                "img.jpg not found. Please keep img.jpg in the same folder as main.py."
            )
    ...
    def generate_differences(self):
        ...
        while len(self.differences) < self.stats.total_differences:
            ...
            diff = Difference(x, y, size, alteration_type)
    ...
    def apply_alteration(self, diff):
        ...
        area = self.modified[y:y + size, x:x + size]

        if diff.alteration_type == "brightness":
            changed = cv2.convertScaleAbs(area, alpha=1.45, beta=35)
            self.modified[y:y + size, x:x + size] = changed

        elif diff.alteration_type == "blur":
            changed = cv2.GaussianBlur(area, (31, 31), 0)
            self.modified[y:y + size, x:x + size] = changed

        elif diff.alteration_type == "colour_shift":
            area[:, :, 1] = cv2.add(area[:, :, 1], 75)
            area[:, :, 2] = cv2.subtract(area[:, :, 2], 25)
            self.modified[y:y + size, x:x + size] = area
    ...
    def check_click(self, event):
        ...
        if clicked_correct:
            if self.remaining_differences() == 0:
                ...
                self.update_status("Correct difference found!")

        else:
            self.stats.mistakes += 1

            if self.stats.mistakes >= self.stats.max_mistakes:
                self.update_status("Game over. You made 3 mistakes.")
                messagebox.showwarning(
                    "Game Over",
                    "You made 3 mistakes. Click Reveal to see the answers."
                )
            else:
                self.update_status("Wrong click!")
    ...
    def draw_found_circle(self, diff):
        ...
        cv2.circle(self.original, center, 28, (0, 0, 255), 2)
        cv2.circle(self.modified, center, 28, (0, 0, 255), 2)
    ...
    def reveal_answers(self):
        ...
        for diff in self.differences:
            if not diff.found:
                ...
                cv2.circle(self.original, center, 28, (255, 0, 0), 2)
                cv2.circle(self.modified, center, 28, (255, 0, 0), 2)

        self.update_images()
        self.update_status("Blue circles show the missing differences.")
    ...
    def restart_game(self):
        ...
        self.generate_differences()
        self.update_images()
        self.update_status(f"Level {self.stats.level} started.")
    ...
    def update_images(self):
        ...
        modified_rgb = cv2.cvtColor(self.modified, cv2.COLOR_BGR2RGB)

        original_pil = Image.fromarray(original_rgb)
        modified_pil = Image.fromarray(modified_rgb)

        self.original_display = ImageTk.PhotoImage(original_pil)
        self.modified_display = ImageTk.PhotoImage(modified_pil)

        self.left_label.config(image=self.original_display)
        self.right_label.config(image=self.modified_display)
